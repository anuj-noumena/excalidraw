...
import { DEFAULT_VERSION, ENV, FONT_FAMILY, getFontFamilyFallbacks, isDarwin, WINDOWS_EMOJI_FALLBACK_FONT } from './constants';
...
export const isToolIcon = (target: Element | EventTarget | null): target is HTMLElement => target instanceof HTMLElement && target.className.includes('ToolIcon');

export const isInputLike = (target: Element | EventTarget | null): target is HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement | HTMLBRElement | HTMLDivElement =>
...
export const isWritableElement = (target: Element | EventTarget | null): target is HTMLInputElement | HTMLTextAreaElement | HTMLBRElement | HTMLDivElement =>
    (target instanceof HTMLElement && target.dataset.type === 'wysiwyg') ||
    target instanceof HTMLBRElement || // newline in wysiwyg
    target instanceof HTMLTextAreaElement ||
    (target instanceof HTMLInputElement && (target.type === 'text' || target.type === 'number' || target.type === 'password'));
...
export const getGlobalCSSVariable = (name: string) => getComputedStyle(document.documentElement).getPropertyValue(`--${name}`);
...
export const findIndex = <T>(array: readonly T[], cb: (element: T, index: number, array: readonly T[]) => boolean, fromIndex: number = 0) => {
    if (fromIndex < 0) {
...
export const findLastIndex = <T>(array: readonly T[], cb: (element: T, index: number, array: readonly T[]) => boolean, fromIndex: number = array.length - 1) => {
    if (fromIndex < 0) {
        fromIndex = array.length + fromIndex;
    }
...
export const mapFind = <T, K>(collection: readonly T[], iteratee: (value: T, index: number) => K | undefined | null): K | undefined => {
...
export const isBindingFallthroughEnabled = (el: ExcalidrawBindableElement) => el.fillStyle !== 'solid' || isTransparent(el.backgroundColor);
...
export const queryFocusableElements = (container: HTMLElement | null) => {
    const focusableElements = container?.querySelectorAll<HTMLElement>('button, a, input, select, textarea, div[tabindex], label[tabindex]');

    return focusableElements ? Array.from(focusableElements).filter((element) => element.tabIndex > -1 && !(element as HTMLInputElement).disabled) : [];
};
...
export const composeEventHandlers = <E>(originalEventHandler?: (event: E) => void, ourEventHandler?: (event: E) => void, { checkForDefaultPrevented = true } = {}) => {
...
export const isMemberOf = <T extends string>(
    ...
    return collection instanceof Set || collection instanceof Map ? collection.has(value as T) : 'includes' in collection ? collection.includes(value as T) : collection.hasOwnProperty(value);
};
...
// Window
export function addEventListener<K extends keyof WindowEventMap>(target: Window & typeof globalThis, type: K, listener: (this: Window, ev: WindowEventMap[K]) => any, options?: boolean | AddEventListenerOptions): UnsubscribeCallback;
export function addEventListener(target: Window & typeof globalThis, type: string, listener: (this: Window, ev: Event) => any, options?: boolean | AddEventListenerOptions): UnsubscribeCallback;
// Document
export function addEventListener<K extends keyof DocumentEventMap>(target: Document, type: K, listener: (this: Document, ev: DocumentEventMap[K]) => any, options?: boolean | AddEventListenerOptions): UnsubscribeCallback;
export function addEventListener(target: Document, type: string, listener: (this: Document, ev: Event) => any, options?: boolean | AddEventListenerOptions): UnsubscribeCallback;
// FontFaceSet (document.fonts)
export function addEventListener<K extends keyof FontFaceSetEventMap>(target: FontFaceSet, type: K, listener: (this: FontFaceSet, ev: FontFaceSetEventMap[K]) => any, options?: boolean | AddEventListenerOptions): UnsubscribeCallback;
// HTMLElement / mix
...
export function addEventListener(
     ...
     */
    target: Document | (Window & typeof globalThis) | FontFaceSet | HTMLElement | undefined | null | false,
...
export const promiseTry = async <TValue, TArgs extends unknown[]>(fn: (...args: TArgs) => PromiseLike<TValue> | TValue, ...args: TArgs): Promise<TValue> => {
    return new Promise((resolve) => {
        resolve(fn(...args));
    });
};

export const isAnyTrue = (...args: boolean[]): boolean => Math.max(...args.map((arg) => (arg ? 1 : 0))) > 0;
...
export const sizeOf = (value: readonly unknown[] | Readonly<Map<string, unknown>> | Readonly<Record<string, unknown>> | ReadonlySet<unknown>): number => {
    return isReadonlyArray(value) ? value.length : value instanceof Map || value instanceof Set ? value.size : Object.keys(value).length;
};

export const reduceToCommonValue = <T, R = T>(collection: readonly T[] | ReadonlySet<T>, getValue?: (item: T) => R): R | null => {
