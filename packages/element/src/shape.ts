...
import { type GeometricShape, getClosedCurveShape, getCurveShape, getEllipseShape, getFreedrawShape, getPolygonShape } from '@excalidraw/utils/shape';

import { assertNever, COLOR_PALETTE, isTransparent, LINE_POLYGON_POINT_MERGE_DISTANCE, ROUGHNESS } from '@excalidraw/common';
import { type LocalPoint, pointDistance, pointFrom, pointRotateRads } from '@excalidraw/math';
...
import { getArrowheadPoints, getCenterForBounds, getDiamondPoints, getElementAbsoluteCoords } from './bounds';
...
export class ShapeCache {
    ...
    public static get = <T extends ExcalidrawElement>(element: T) => {
        return ShapeCache.cache.get(element) as T['type'] extends keyof ElementShapes ? ElementShapes[T['type']] | undefined : ElementShape | undefined;
    };

    public static set = <T extends ExcalidrawElement>(element: T, shape: T['type'] extends keyof ElementShapes ? ElementShapes[T['type']] : Drawable) => ShapeCache.cache.set(element, shape);
...
const modifyIframeLikeForRoughOptions = (element: NonDeletedExcalidrawElement, isExporting: boolean, embedsValidationStatus: EmbedsValidationStatus | null) => {
...
const getArrowheadShapes = (element: ExcalidrawLinearElement, shape: Drawable[], position: 'start' | 'end', arrowhead: Arrowhead, generator: RoughGenerator, options: Options, canvasBackgroundColor: string) => {
    ...
    switch (arrowhead) {
        ...
        default: {
            ...
            return [generator.line(x3, y3, x2, y2, options), generator.line(x4, y4, x2, y2, options), ...(arrowhead === 'crowfoot_one_or_many' ? generateCrowfootOne(getArrowheadPoints(element, shape, position, 'crowfoot_one'), options) : [])];
        }
    }
};

export const generateLinearCollisionShape = (element: ExcalidrawLinearElement | ExcalidrawFreeDrawElement) => {
    ...
    const center = getCenterForBounds(
        ...
        element.points.reduce(
            (acc, point) => {
                return [Math.min(element.x + point[0], acc[0]), Math.min(element.y + point[1], acc[1]), Math.max(element.x + point[0], acc[2]), Math.max(element.y + point[1], acc[3])];
            },
        ...
        case 'arrow': {
            ...
            if (isElbowArrow(element)) {
                ...
                return points.map((point, idx) => {
                    const p = pointRotateRads(pointFrom<GlobalPoint>(element.x + point[0], element.y + point[1]), center, element.angle);
            ...
            return generator
                ...
                .map((op, i) => {
                    if (i === 0) {
                        const p = pointRotateRads<GlobalPoint>(pointFrom<GlobalPoint>(element.x + op.data[0], element.y + op.data[1]), center, element.angle);
                    ...
                    return {
                        op: 'bcurveTo',
                        data: [
                            pointRotateRads(pointFrom<GlobalPoint>(element.x + op.data[0], element.y + op.data[1]), center, element.angle),
                            pointRotateRads(pointFrom<GlobalPoint>(element.x + op.data[2], element.y + op.data[3]), center, element.angle),
                            pointRotateRads(pointFrom<GlobalPoint>(element.x + op.data[4], element.y + op.data[5]), center, element.angle)
                        ]
...
const generateElementShape = (
    ...
    switch (element.type) {
        ...
        case 'ellipse': {
            const shape: ElementShapes[typeof element.type] = generator.ellipse(element.width / 2, element.height / 2, element.width, element.height, generateRoughOptions(element));
            return shape;
        }
        case 'line':
        case 'arrow': {
            ...
            if (isElbowArrow(element)) {
                ...
                if (!points.every((point) => Math.abs(point[0]) <= 1e6 && Math.abs(point[1]) <= 1e6)) {
                    ...
                    shape = [generator.path(generateElbowArrowShape(points, 16), generateRoughOptions(element, true))];
                }
            } else if (!element.roundness) {
 ...
 */
export const getElementShape = <Point extends GlobalPoint | LocalPoint>(element: ExcalidrawElement, elementsMap: ElementsMap): GeometricShape<Point> => {
    switch (element.type) {
        case 'rectangle':
        case 'diamond':
        case 'frame':
        ...
        case 'line': {
            ...
            return shouldTestInside(element)
                ? getClosedCurveShape<Point>(element, roughShape, pointFrom<Point>(element.x, element.y), element.angle, pointFrom(cx, cy))
                : getCurveShape<Point>(roughShape, pointFrom<Point>(element.x, element.y), element.angle, pointFrom(cx, cy));
        }
