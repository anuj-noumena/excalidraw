...
const ALLOW_SAME_ORIGIN = new Set(['youtube.com', 'youtu.be', 'vimeo.com', 'player.vimeo.com', 'figma.com', 'twitter.com', 'x.com', '*.simplepdf.eu', 'stackblitz.com', 'reddit.com', 'forms.microsoft.com']);
...
export const getEmbedLink = (link: string | null | undefined): IframeDataWithSandbox | null => {
    ...
    if (RE_TWITTER.test(link)) {
        ...
        const ret: IframeDataWithSandbox = {
            type: 'document',
            srcdoc: (theme: string) =>
                createSrcDoc(`<blockquote class="twitter-tweet" data-dnt="true" data-theme="${theme}"><a href="${safeURL}"></a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>`),
    ...
    if (RE_REDDIT.test(link)) {
        ...
        const ret: IframeDataWithSandbox = {
            type: 'document',
            srcdoc: (theme: string) => createSrcDoc(`<blockquote class="reddit-embed-bq" data-embed-theme="${theme}"><a href="${safeURL}"></a><br></blockquote><script async="" src="https://embed.reddit.com/widgets.js" charset="UTF-8"></script>`),
            intrinsicSize: { w: 480, h: 480 },
            sandbox: { allowSameOrigin }
        };
        embeddedLinkCache.set(originalLink, ret);
...
export const embeddableURLValidator = (url: string | null | undefined, validateEmbeddable: ExcalidrawProps['validateEmbeddable']): boolean => {
    if (!url) {
        return false;
    }
